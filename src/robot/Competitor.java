...
public class Competitor {
	...
	public static void main(String[] args) throws InterruptedException {
		 ...
		 * 
		 * // example usage of Transmission class Transmission t =
		 * conn.getTransmission();
		 * 
		 * if (t == null) { LCD.drawString("Failed to read transmission", 0, 5);
		 * } else { corner = t.startingCorner; role = t.role;
		 * 
		 ...
		 * 
		 * 
		 * if(role.getId()==1){ SensorMotorUser.becomeBuilder(true); }
		 * 
		 * else if(role.getId()==2){ SensorMotorUser.becomeBuilder(false); }
		 */

		/*
		 * if(SensorMotorUser.isBuilder()){ Map.setTargetZone(greenZone);
		 * Map.setForbiddenZone(redZone); }
		 * 
		 * else if(!SensorMotorUser.isBuilder()){ Map.setTargetZone(redZone);
