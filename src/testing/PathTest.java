...
public class PathTest {
	...
	public static void main(String[] args) {
		...
		for (int initialRow = rowNumber; loopCondition(rowNumber, initialRow); loopAfterthought(initialRow)) {
			...
			System.out.println(endOfCurrentRow().getX() + " " + endOfCurrentRow().getY());
	...
	private static Intersection nextRow(){
		if(SensorMotorUser.getXStart()==0 && SensorMotorUser.getYStart()==0 && rowNumber!=Map.NUM_OF_INTERSECTIONS-1){
			return getNextDestination(rowNumber+1);
		}
		
		else if(SensorMotorUser.getXStart()==0 && SensorMotorUser.getYStart()== Map.NUM_OF_INTERSECTIONS-1 && rowNumber!=Map.NUM_OF_INTERSECTIONS-1){
			return getNextDestination(rowNumber+1);
		}
		
		else if(SensorMotorUser.getXStart()==Map.NUM_OF_INTERSECTIONS-1 && SensorMotorUser.getYStart()== Map.NUM_OF_INTERSECTIONS-1 && rowNumber!=0){
			return getNextDestination(rowNumber-1);
		}
		
		else if(SensorMotorUser.getXStart()==Map.NUM_OF_INTERSECTIONS-1 && SensorMotorUser.getYStart()== 0 && rowNumber!=0){
			return getNextDestination(rowNumber-1);
		}
	...
	private static Intersection getValidIntersection(int tempRowNumber, int defaultDestination, boolean yIsRow){
	...
	private static Intersection getNextDestination(int tempRowNumber){
		...
		if(rowCounter%2 == 0){
			...
			if(SensorMotorUser.getXStart()==0 && SensorMotorUser.getYStart()==0){
				return getValidIntersection(tempRowNumber,Map.NUM_OF_INTERSECTIONS,true);
			}
			
			else if(SensorMotorUser.getXStart() == 0 && SensorMotorUser.getYStart()==Map.NUM_OF_INTERSECTIONS-1){
				return getValidIntersection(tempRowNumber,1,false);
			}
			
			else if(SensorMotorUser.getXStart() == Map.NUM_OF_INTERSECTIONS-1 && SensorMotorUser.getYStart()==Map.NUM_OF_INTERSECTIONS-1){
				return getValidIntersection(tempRowNumber,1,true);
			}
			
			else if(SensorMotorUser.getXStart() == Map.NUM_OF_INTERSECTIONS-1 && SensorMotorUser.getYStart()==0){
